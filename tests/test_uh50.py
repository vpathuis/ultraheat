...
class HeatMeterTest(unittest.TestCase):
    def assert_response_data(self, response_data):
        ...
        self.assertEqual(98.5, response_data.flow_temperature_max_previous_year_c)
        self.assertEqual(96.1, response_data.return_temperature_max_previous_year_c)
        self.assertEqual("01 00:00", response_data.monthly_set_day)
        self.assertEqual(
            datetime(2022, 5, 19, 19, 41, 17), response_data.meter_date_time
        )
        self.assertEqual(1.5, response_data.measuring_range_m3ph)
        self.assertEqual(
            "0 1 0 0000 CECV CECV 1 5.16 5.16 F 101008 040404 08 0",
            response_data.settings_and_firmware,
        )
        self.assertEqual(28849, response_data.flow_hours)

    @patch("serialx.serial_for_url")
    def test_read_port(self, mock_serial_for_url):
        mock_serial_for_url().__enter__().readline.side_effect = mock_readline
    ...
    def test_heat_meter_read_file(self):
        heat_meter_service = HeatMeterService(FileReader(dummy_file_path))
        response_data = heat_meter_service.read()
        self.assert_response_data(response_data)
    ...
    @patch("serialx.serial_for_url")
    def test_read_port_timeout(self, mock_serial_for_url):
        ...
        mock_serial_for_url().__enter__().readline.side_effect = mock_readline_timeout
        reader = UltraheatReader(DUMMY_PORT)
